...
### Added
- **`decay serve`** — a self-contained dashboard embedded in the binary, over a read-only GET-only JSON API. Shows the decay trend, the latest run, and each row's full gate breakdown on click. Binds to loopback unless `--allow-remote`: there is no authentication, and the page is a map of where an estate's detection is blind. It renders no verdict at all for a run that measured nothing, and warns when the newest *attempt* was not indexed — otherwise an hours-old healthy board displays as though it were current, which is the same false reassurance the tool exists to prevent. Pattern from purple-loop's `serve`.
- **`decay calibrate`** derives baselines from recorded run history, and `decay score --baselines` applies them to rows carrying none. Baselines are the median of a window — one outlier run must not move them, and outliers are what a decaying pipeline produces. **Only observations that were themselves `HEALTHY` contribute:** a rolling baseline computed from every observation follows a dead source down until a total outage reports perfectly healthy, a failure that is silent and permanent and therefore worse than no calibration at all. An explicit baseline in the evidence file always wins, and the baselines file's age flows into confidence rather than the score.
- **`scripts/sigma-to-rules.py`** derives a rules file from real Sigma rules — required fields, logsource, ATT&CK technique tags — instead of hand-declaring `--field` one rule at a time. Handles the three selection shapes Sigma allows (field map, list of maps, bare keyword list) and strips value modifiers. Sigma is YAML and the binary is stdlib-only, so the conversion lives in the script and the binary reads JSON; this is the only place PyYAML is needed. Unmapped fields are reported, never guessed at — a fabricated field name measures 0% populate forever and reads as permanent, confident field drift. Extraction logic borrowed from purple-loop's `RuleParser`.
...
### Fixed
- **Run ids sorted lexicographically.** Collision suffixes run `-2`, `-3` … `-10`, and `"Z-10" < "Z-2"` as strings, so once ten runs landed in the same second the index was mis-ordered: `Latest` returned the wrong run and the run-over-run diff compared against the wrong baseline. Ordering is now by timestamp then numeric suffix.
